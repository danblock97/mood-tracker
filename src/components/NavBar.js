...
const NavBar = () => {
  ...
  return (
    <>
      ...
      <nav
        className={`bg-white z-10 pt-4 fixed h-full md:w-60 w-64 p-4 shadow-lg transition-all duration-300 ease-in-out ${
          isSidebarOpen ? 'left-0' : '-left-64 md:left-0'
        }`}
      >
        <div className="flex flex-col items-center md:items-start space-y-4">
          <Link href="/" className="text-blue-600 text-lg font-bold flex items-center mb-4 md:hidden" onClick={handleLinkClick}>
          ...
          {user ? (
            <>
              ...
              <Link href="/account" className="flex items-center space-x-2 w-full" onClick={handleLinkClick}>
                ...
                <Link href="mailto:danblock1997@hotmail.co.uk" className="flex items-center space-x-2 w-full" onClick={handleLinkClick}>
                <RiFeedbackLine className="text-blue-600" />
                <span className="hidden md:block text-gray-900">Feedback & Support</span>
                {isSidebarOpen && <span className="md:hidden text-gray-900">Feedback & Support</span>}
              </Link>
